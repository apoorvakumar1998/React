  ...
  return (
    <div className="body-container p-6">
      ...
      <div className="res-wrapper flex mx-32 gap-6 flex-wrap items-start justify-center mt-16">
        {dummyList.map((data, index) => <Link className='res-card inline-flex gap-3 flex-col no-underline rounded-lg transition-transform duration-300 ease-in-out w-1/4 hover:cursor-pointer hover:scale-105' key={data.info?.id || index} to={'restaurent/' + data.info?.id}>
          {data.info.promoted ?
